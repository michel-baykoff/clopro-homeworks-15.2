...
#!/bin/bash
cd /var/www/html
echo '<html><head><title>Cat from storage</title></head><body><h1>Cat in the hat is about to open storage object</h1><img src="http://${yandex_storage_bucket.netology_bucket.bucket_domain_name}/cat_in_the_hat.jpg"/></body></html>' > index.html
EOF
    }
  }
